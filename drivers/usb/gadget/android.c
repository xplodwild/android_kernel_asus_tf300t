...
static void volume_up_and_down_gpios_init(void)
{
	int retval = 0;

	tegra_gpio_enable(TEGRA_GPIO_PQ2);
	tegra_gpio_enable(TEGRA_GPIO_PQ3);

	retval = gpio_request(TEGRA_GPIO_PQ3, "VOL_DWN_BUTTON");
	if (retval < 0)
		printk(KERN_ERR"%s:GPIO%d request fault!%d\n", __func__, TEGRA_GPIO_PQ3, retval);

	retval = gpio_request(TEGRA_GPIO_PQ2, "VOL_UP_BUTTON");
	if (retval < 0)
		printk(KERN_ERR"%s:GPIO%d request fault!%d\n", __func__, TEGRA_GPIO_PQ2, retval);

	retval = gpio_direction_input(TEGRA_GPIO_PQ2);
	if (retval) {
		printk(KERN_ERR"%s:gpio_direction_input failed for input %d\n", __func__, TEGRA_GPIO_PQ2);
	}
	retval = gpio_direction_input(TEGRA_GPIO_PQ3);
	if (retval) {
		printk(KERN_ERR"%s:gpio_direction_input failed for input %d\n", __func__, TEGRA_GPIO_PQ3);
	}
}
